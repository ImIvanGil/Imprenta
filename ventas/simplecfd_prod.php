...
class SimpleCFD {
  ...
  public static function getXML ( array $data )
  {
    ...
    // Comprobante
    $co = $dom->createElement( 'cfdi:Comprobante' );
    $dom->appendChild( $co );

    $xsi = $dom->createAttribute( 'xmlns:cfdi' );
    $xsi->appendChild( $dom->createTextNode( "http://www.sat.gob.mx/cfd/3" ) );
    $co->appendChild( $xsi );
	
	$xsi_sche = $dom->createAttribute( 'xsi:schemaLocation' );
    $xsi_sche->appendChild( $dom->createTextNode( "http://www.sat.gob.mx/cfd/3 http://www.sat.gob.mx/sitio_internet/cfd/3/cfdv32.xsd" ) );
    $co->appendChild( $xsi_sche );
    ...
    if ( isset( $data['formaDePago'] ) ) {
	 ...
	 if ( isset( $data['metodoDePago'] ) ) {
      $metodoDePago = $dom->createAttribute( 'metodoDePago' );
      $co->appendChild( $metodoDePago );
      $metodoDePago->appendChild( $dom->createTextNode( $data['metodoDePago'] ) );
    }
	 if ( isset( $data['LugarExpedicion'] ) ) {
      $lugarExpedicion = $dom->createAttribute( 'LugarExpedicion' );
      $co->appendChild( $lugarExpedicion );
      $lugarExpedicion->appendChild( $dom->createTextNode( $data['LugarExpedicion'] ) );
    }
	
	if ( isset( $data['NumCtaPago'] ) ) {
      $NumCtaPago = $dom->createAttribute( 'NumCtaPago' );
      $co->appendChild( $NumCtaPago );
      $NumCtaPago->appendChild( $dom->createTextNode( $data['NumCtaPago'] ) );
    }
    ...
    if ( isset( $data['tipoCambio'] ) ) {
      $tipoCambio = $dom->createAttribute( 'TipoCambio' );
      $co->appendChild( $tipoCambio );
      $tipoCambio->appendChild( $dom->createTextNode( $data['tipoCambio'] ) );
    }
    if ( isset( $data['moneda'] ) ) {
      $moneda = $dom->createAttribute( 'Moneda' );
      $co->appendChild( $moneda );
      $moneda->appendChild( $dom->createTextNode( $data['moneda'] ) );
    }
    ...
    // Emisor
    $e = $dom->createElement( 'cfdi:Emisor' );
    $co->appendChild( $e );
    ...
    // DomicilioFiscal
    $df = $dom->createElement( 'cfdi:DomicilioFiscal' );
    $e->appendChild( $df );
    ...
    if ( isset( $data['ExpedidoEn'] ) ) {
      ...
      $en = $dom->createElement( 'cfdi:ExpedidoEn' );
      $e->appendChild( $en );
	 ...
	 // Regimen
    $re = $dom->createElement( 'cfdi:RegimenFiscal' );
    $e->appendChild( $re );

    if ( isset( $data['RegimenFiscal']['Regimen'] ) ) {
      $re_regimen = $dom->createAttribute( 'Regimen' );
      $re->appendChild( $re_regimen );
      $re_regimen->appendChild( $dom->createTextNode( $data['RegimenFiscal']['Regimen'] ) );
    }

    

    // Receptor
    $r = $dom->createElement( 'cfdi:Receptor' );
    $co->appendChild( $r );
    ...
    // Domicilio
    $d = $dom->createElement( 'cfdi:Domicilio' );
    $r->appendChild( $d );
    ...
    // Conceptos
    $cs = $dom->createElement( 'cfdi:Conceptos' );
    $co->appendChild( $cs );
    ...
    if ( $count == 1 ) {
      ...
      for( $i = 0; $i < $count; ++$i ) {
        ...
        $c_{$i} = $dom->createElement( 'cfdi:Concepto' );
        $cs->appendChild( $c_{$i} );
    ...
    // Impuestos
    $im = $dom->createElement( 'cfdi:Impuestos' );
    $co->appendChild( $im );

    // Retenciones
    if ( isset( $data['Retencion'] ) ) {
      $rs = $dom->createElement( 'cfdi:Retenciones' );
      $im->appendChild( $rs );
      ...
      if ( $count == 1 ) {
        ...
        $rt = $dom->createElement( 'cfdi:Retencion' );
        $rs->appendChild( $rt );
        ...
        for( $i = 0; $i < $count; ++$i ) {
          ...
          $rt_{$i} = $dom->createElement( 'cfdi:Retencion' );
    ...
    if ( isset( $data['Traslado'] ) ) {
      ...
      $ts = $dom->createElement( 'cfdi:Traslados' );
      $im->appendChild( $ts );
      ...
      if ( $count == 1 ) {
        ...
        $tr = $dom->createElement( 'cfdi:Traslado' );
        $ts->appendChild( $tr );
        ...
        for( $i = 0; $i < $count; ++$i ) {
          ...
          $tr_{$i} = $dom->createElement( 'cfdi:Traslado' );
	  ...
	  //seccion para el complemento
		//$comp = $dom->createElement( 'cfdi:Complemento' );
    	//$co->appendChild( $comp );
  ...
  public static function getOriginalString ( array &$data )
  {
    ...
    $string .= isset( $data['tipoCambio']  ) ? $data['tipoCambio'].'|' : '';
 	$string .= isset( $data['moneda']  ) ? $data['moneda'].'|' : '';
    $string .= isset( $data['total']  ) ? $data['total'].'|' : '';
	$string .= isset( $data['metodoDePago']  ) ? $data['metodoDePago'].'|' : '';
	$string .= isset( $data['LugarExpedicion']  ) ? $data['LugarExpedicion'].'|' : '';
	$string .= isset( $data['NumCtaPago']  ) ? $data['NumCtaPago'].'|' : '';
	...
	//regimen fiscal
	$string .= isset( $data['RegimenFiscal']['Regimen']) ? $data['RegimenFiscal']['Regimen'].'|' : '';
    ...
    if ( isset( $data['Traslado'] ) ) {
      ...
      unset( $count );
      $string .= isset( $data['TTraslados']['totalImpuestosTraslados'] ) ? $data['TTraslados']['totalImpuestosTraslados'].'|' : '';
    }
  ...
  {
    $cmd = '"C:\wamp\openssl\bin\openssl" pkcs8 -inform DER -in '.$key_path.' -passin pass:'.$password;
	//$cmd = 'openssl pkcs8 -inform DER -in '.$key_path.' -passin pass:'.$password;
    //echo "el comando del get private key:".$cmd."<br>";
    if ( $result = shell_exec( $cmd ) ) {
      unset( $cmd );

      //echo "el resultado del get private key:".$result."'<br>";
      return $result;
    }
    //echo "el resultado del get private key:".$result."'<br>";
    return false;
  ...
  {
    $cmd = '"C:\wamp\openssl\bin\openssl" x509 -inform DER -outform PEM -in '.$cer_path.' -pubkey';
  ...
  {
    $cmd = '"C:\wamp\openssl\bin\openssl" x509 -inform DER -outform PEM -in '.$cer_path.' -pubkey | '.
           '"C:\wamp\openssl\bin\openssl" x509 -serial -noout';
	//$cmd = 'openssl x509 -inform DER -outform PEM -in '.$cer_path.' -pubkey | '.
