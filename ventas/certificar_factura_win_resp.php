...
while ($row = $db->fetch_array($factura))
{
		...
		if($existe<=0){
		  ...
		  // ************  AHORA VOY A ASIGNAR LOS VALORES A UN ARREGLO PARA LLAMAR AL LOS METODOS Y GENERAR EL XML Y LA CADENA Y TODO
			...
			//require_once 'simple_CFD_original.php';
			
			// Variables que indican donde estan las claves
			$password = 'Cpimpresos2011';
			/*$cer_path = 'http://localhost/imprenta/firmas/igc061204e47.cer';
			$key_path = 'http://localhost/imprenta/firmas/igc061204e47_1103281301.key';*/
			$cer_path = 'firmas/igc061204e47.cer';
			$key_path = 'firmas/igc061204e47_1109271035s.key';
			
			/*$password = 'a0123456789';
			$cer_path = 'firmas/aaa010101aaa_CSD_10.cer';
			$key_path = 'firmas/aaa010101aaa_CSD_10.key';*/
			//echo "el password : ".$password."<br>";
			//echo "ruta del certificado : ".$cer_path."<br>";
			//echo "ruta del key : ".$key_path."<br>";
			...
			$file = "cfdi/xml/prueba".$rfc_emp."_".$clave.".xml";
			...
			/*****************Llamamos a la librería de Web Services************************/
			...
			$wsdl = 'https://test.paxfacturacion.com.mx/webservices/wcfRecepcion.svc?wsdl';
			//$wsdl = 'https://test.paxfacturacion.com.mx/webservices/wcfRecepcionASMX.asmx?wsdl';
			...
			'estructura'=>0, 
			'usuario'=>'igraficos', 
			'password'=>'w4TDjMOTw7rEgsS2w6jElMSrxLRrTMKBG8KAczBuwqVU77+G77+iFe+/j+++pu+9nu+/qO+/qu++sg==', 
			'version'=>'3.2'
			); 
			*/
			$function = "fnEnviarXML";
			$meter = "\"".$xmlToText."\"";
			$params = array('psComprobante' => $xmlToText,'psTipoDocumento' => 'factura', 'pnId_Estructura' => 0,'sNombre' => 'igraficos', 'sContrasena' => 'w4TDjMOTw7rEgsS2w6jElMSrxLRrTMKBG8KAczBuwqVU77+G77+iFe+/j+++pu+9nu+/qO+/qu++sg==','sVersion' => '3.2');
			...
			$respuesta = $soapclient->fnEnviarXML($params);
			
			
			//if (!$error = $soapclient->getError()){

				echo "<br>+++++++++++la respuesta es: <br>";
				print_r ($respuesta);
				
				$respuesta = $respuesta["TimbrarCFDResult"]["string"][0];
				if($desc_error==""){
					$XMLS=$respuesta["TimbrarCFDResult"]["string"][0];
					$file_cer = "cfdi/xml_cer/PAX".$rfc_emp."_".$clave.".xml";
					...
					$cosa =  $dom->saveXml();
					$file = "cfdi/xml_cer/PAX".$rfc_emp."_".$clave.".xml";
					...
					$query = "UPDATE factura SET no_serie_cert='$no_serie_cert',fecha_certificacion='$fecha_cert',sello_sat='$sello_sat',cadena_cert='$cadena_cert',uuid='$uuid',id_status_factura='2',sello='$sello',id_status_cobranza='$estado_cobro' WHERE id_factura ='$clave';";
