...
<div id="templatemo_wrapper">
    ...
    <div id="templatemo_service_bar">
    	...
    	<div class="sb_box sb_box_last">
         ...
         <div class="sb_box">
            ...
            <a href="<?php echo "rep_vtas_vend_excel.php?inicio=$inicio&fin=$fin&id_vend=$id_vendedor";?>">Exportar a MS Excel</a>
    ...
    <div id="templatemo_content">
    	...
    	<div class="col_w565_interior">
			...
			<?php
				//consulto los datos del vendedor
				$consulta_vende = $db->consulta("SELECT *  FROM `empleado` WHERE `id_empleado` = '".$id_vendedor."';");
				while ($row2 = $db->fetch_array($consulta_vende))
				{
					$vendedor = utf8_decode($row2['nombre']);
					$porc_comi = $row2['comision'];
				}
				$finicio = cambiarFormatoFecha($inicio);
				$ffin = cambiarFormatoFecha($fin);
				echo "<p>Para el vendedor <b>$vendedor</b> del <b>$finicio</b> al <b>$ffin</b></p>";
				//consulto los pagos recibidos en el periodo en pesos mexicanos por los clientes del vendedor
				$cons_pagos_mx = "SELECT sum(`monto`) as suma FROM `pago_cliente` WHERE `id_moneda`='1' AND `fecha` BETWEEN '$inicio' AND '$fin' AND `id_cliente` IN (SELECT `id_cliente` FROM `cliente` WHERE `id_vendedor` = '$id_vendedor')";
				...
				//consulto los pagos recibidos en el en dolares por los clientes de este vendedor
				$cons_pagos_us = "SELECT sum(`monto`) as suma FROM `pago_cliente` WHERE `id_moneda`='2' AND `fecha` BETWEEN '$inicio' AND '$fin' AND `id_cliente` IN (SELECT `id_cliente` FROM `cliente` WHERE `id_vendedor` = '$id_vendedor')";
				...
				//consulto las facturas del periodo para los clientes del vendedor
				$lista = $db->consulta("SELECT * FROM `factura` WHERE `fecha` between '$ninicio' and '$nfin' and `id_status_factura`='2' AND `id_cliente` IN (SELECT `id_cliente` FROM `cliente` WHERE `id_vendedor` = '$id_vendedor')");
				...
				<?php 
					...
					//consulto la tasa de IVA normal para aplicarla a las comisiones
					$sql_tas = "SELECT tasa FROM iva WHERE id_iva='1'";
					$consulta_tas = $db->consulta($sql_tas);
					$row_tas = mysql_fetch_array($consulta_tas);
					$tasa_iva_comi = $row_tas['tasa'];
							
					$comi = $porc_comi * 100;
					$factor = 1+($tasa_iva_comi/100);
					$base_comi = $pagos_total/$factor;
					$comision_cobrado= $base_comi * $porc_comi;
					...
					<tr>
						<td align=\"right\"><b>Base para Comisi&oacute;n: $</b></td><td><b> ".number_format($base_comi,2)."</b></td>
						<td align=\"right\"><b>Comisi&oacute;n: </b></td><td><b> ".number_format($comi,2)."%</b></td>
						<td align=\"right\"><b>Comisi&oacute;n sobre cobranza<br> sin IVA: $ </b></td><td><b><br> ".number_format($comision_cobrado,2)."</b></td>
					</tr>
					<tr>
