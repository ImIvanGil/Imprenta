...
while ($row = $db->fetch_array($factura))
{
		...
		if($existe<=0){
		  ...
		  // ************  AHORA VOY A ASIGNAR LOS VALORES A UN ARREGLO PARA LLAMAR AL LOS METODOS Y GENERAR EL XML Y LA CADENA Y TODO
			...
			//$Lectura = fread($Apertura, filesize($file)); 
			$Lectura = $cfdi;
			$xmlToText = htmlentities($Lectura,ENT_NOQUOTES,"UTF-8");
			//$xmlToText = $Lectura;
			$xmlToText = trim($xmlToText);
			//$xmlToText = "'".$xmlToText."'";
			//$xmlToText = utf8_decode($xmlToText);
			/*echo "filesize del XML<br>";
			print(filesize($file));
			echo "<br>";
			...
			/*echo "****CONFIRMA SI LA CADENA DEL XML ES UTF8****<br>";
			require_once('isUTF8.php');
			$esUTF = isUTF8($xmlToText);
			if(!$esUTF){
					echo "La cadena NO es utf8 <br>";
				}else{
					echo "La cadena SI es utf8 <br>";	
				}*/
			
			//ahora voy a enviar el cfdi a folios digitales para obtener el sello 
			//lo dejare comentado hasta probar con un server en linea mientras tanto simulare tener ya los datos para acabar de crear el pdf
			...
			{
			//print_r ($respuesta);
				$codigoError=$respuesta["TimbrarPruebaCFDIResult"]["string"][0];
				$mensajeError=$respuesta["TimbrarPruebaCFDIResult"]["string"][1];
				$complementoError=$respuesta["TimbrarPruebaCFDIResult"]["string"][2];
				if($codigoError==""){
					//si no hay mensajes de error guardo el archivo xml
					echo "*******SE TIMBRO CON EXITO EL CFDI******: <br>";
					$XMLS=$respuesta["TimbrarPruebaCFDIResult"]["string"][3];
					$fileCert = "cfdi/xml_cer/".$rfc_emp."_".$clave.".xml";
					file_put_contents($fileCert, "$XMLS");
					echo "la ruta del xml timbrado es ".$fileCert."<br>";
				}else{
					//recibo el error y lo muestro
					echo "*******SE PRODUJO EL SIGUIENTE ERROR EN LA CONEXION CON FOLIOSDIGITALES******: <br>";
					echo "codigo: ".$codigoError."<br>";
					echo "MENSAJE ERROR: ".$mensajeError."<br>";
					echo "COMPLEMENTO ERROR: ".$complementoError."<br>";
			...
			require_once('lib/simpleXML/IsterXmlSimpleXMLImpl.php');
			 ...
			 // print htmlentities($attr['selloCFD'], ENT_NOQUOTES, 'UTF-8');
			  //print iconv("ISO-8859-1", "UTF-8", $attr['selloCFD']);
			  print $attr['selloCFD'];
			  print "\n";
			...
			//aqui actualizamos la tabla factura y agregamos los datos recibidos del certificado
			...
			//antes generaremos el codigo QR
