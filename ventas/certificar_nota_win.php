...
//obtengo el id de la nota para consultar sus datos
$clave = $_GET['numero'];	
...
$nota = $db->consulta("SELECT * FROM `nota_credito` WHERE id_nota='".$clave."'");
$existe = $db->num_rows($nota);
if($existe<=0){
	echo "No hay informaci&oacute;n de la nota con la clave <b>$clave</b>, verifique sus datos";

}else{

while ($row = $db->fetch_array($nota))
{
		$fecha = $row['fecha'];
		...
		//voy a consultar las lineas de detalle de la nota
		$detalle = $db->consulta("SELECT * FROM `detalle_nota_credito` WHERE id_nota='".$clave."'");
		$existe = $db->num_rows($detalle);
		if($existe<=0){
			echo "<p align=\"center\">No se han agregado productos a la nota</p>";
		  ...
		  $ftotal_nota = number_format($total_nota, 2, '.', '');
		  ...
		  // ************  AHORA VOY A ASIGNAR LOS VALORES A UN ARREGLO PARA LLAMAR AL LOS METODOS Y GENERAR EL XML Y LA CADENA
			...
			$file = "notas/xml/".$rfc_emp."_".$clave.".xml";
			...
			//++++++++++++++++ PARTE DONDE HACE LA CONEXION PARA EL TIMBRADO
			  ...
			  $file_cer = "notas/xml_cer/".$rfc_emp."_".$clave.".xml";
			  ...
			  require_once('../lib/simpleXML/IsterXmlSimpleXMLImpl.php');
					...
					$totalFomat=number_format($total_nota,6, '.', '');
					$cadenaQR = "?re=".$rfc_emp."&rr=".$rfc_cliente."&tt=".$totalFomat."&id=".$uuid;
					//echo $cadenaQR."<br>";
					
					require('../lib/phpqrcode/phpqrcode.php');
					$rutaQR="notas/QR/".$rfc_emp."_".$clave.".png";
					QRcode::png($cadenaQR, $rutaQR);
					
					
					//hago la consulta de update
					$query = "UPDATE nota_credito SET no_serie_cert='$no_serie_cert',fecha_certificacion='$fecha_cert',sello_sat='$sello_sat',cadena_cert='$cadena_cert',uuid='$uuid',id_status_factura='2',sello='$sello',no_certificado ='$no_cert' WHERE id_nota ='$clave';";
