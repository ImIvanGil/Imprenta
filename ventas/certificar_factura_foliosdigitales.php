		...
		if($existe<=0){
		  ...
		  // ************  AHORA VOY A ASIGNAR LOS VALORES A UN ARREGLO PARA LLAMAR AL LOS METODOS Y GENERAR EL XML Y LA CADENA Y TODO
			...
			if (!$error = $oSoapClient->getError())
			{
				...
				print_r ($respuesta);
				$cod_error = $respuesta["TimbrarCFDResult"]["string"][0];
				$desc_error = $respuesta["TimbrarCFDResult"]["string"][1];
				$comp_error = $respuesta["TimbrarCFDResult"]["string"][2];
				if($desc_error==""){
					$XMLS=$respuesta["TimbrarCFDResult"]["string"][3];
					$XMLAC=$respuesta["TimbrarCFDResult"]["string"][4];
					...
					echo "referencia: ".$datos['referencia']."<br>";
