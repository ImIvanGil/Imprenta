...
if (isset($_GET['guardar'])) {
	...
	$query="SELECT * from producto_almacen where id_orden_produccion=".$cve_orden.";";
		...
		$suma_cantidades=$suma_cantidades+$cant;

	$query="SELECT * FROM detalle_orden_produccion where id_orden=".$cve_orden.";";
		//echo $query;
		$query=$db->consulta($query);
		$row4=$db->fetch_array($query);
		if($suma_cantidades>=$row4['cantidad']){
			$query="UPDATE `orden_produccion` SET `id_estado`=3 WHERE id_orden=".$cve_orden.";";
			echo $query;
			$query=$db->consulta($query);
