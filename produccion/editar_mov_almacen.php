   ...
   <div id="templatemo_menu">
    ...
    <div id="templatemo_content">
    	...
    	<div class="col_w565_interior">
            ...
            <h2>Editar Movimiento Almac&eacute;n</h2>
			...
			<?php 
				...
				//DATOS DEL PRODUCTO
				$prod = $db->consulta("SELECT * FROM `producto` WHERE id_producto='".$clave."'");
				while ($row = $db->fetch_array($prod))
				{
					$nombre_prod = utf8_encode($row['nombre']);
				}
				//CONSULTO LOS DATOS DEL MOVIMIENTO
				$movimiento = $db->consulta("SELECT * FROM `producto_almacen` WHERE id_movimiento=$key");
				while ($row3 = $db->fetch_array($movimiento))
				{
					$fecha = $row3['fecha'];
					$tipo_mov = $row3['id_tipo_movimiento'];
					$unidades = $row3['cantidad'];
					$orden = $row3['id_orden_produccion'];
					$unitario = number_format($row3['unitario'],4);
				}
			?> 
         ...
         <tr><td align="right">*Fecha:</td>
          <td> <input class="texto" type="text" id="fecha" name="fecha" size="10" value="<?php echo $fecha;?>" /></td></tr>
          ...
          <?php 
		  if($tipo_mov==1){
			echo "<div id=\"oprod\" style=\"display:block;\">";
		  }else{
		  	echo "<div id=\"oprod\" style=\"display:none;\">";
		  }
		  ?>
          
          
          *Orden de producci&oacute;n: <select id="o_prod" name="o_prod">
                  <option value="-1">Seleccione</option>
                  <?php 
				  ...
				  if($existe<=0){
                      ...
                      $id_orden = $row['id_orden'];
						  if($id_orden == $orden){
							  echo "<option value='".$id_orden."' selected>".$id_orden."</option>";
						  }else{
         ...
         <?php 
		  if($tipo_mov==1){
			echo "<div id=\"cunitario\" style=\"display:block;\">";
		  }else{
		  	echo "<div id=\"cunitario\" style=\"display:none;\">";
		  }
		  ?>
         &nbsp;&nbsp;&nbsp;*Costo unitario: $ <input type="text" class="texto" id="unitario" name="unitario" size="20" value="<?php echo $unitario;?>"></div></td>
         </tr>
          
          <tr>
          <td align="right">*Unidades:</td>
          <td colspan="2"><input type="text" class="texto" id="unidades" name="unidades" size="20" value="<?php echo $unidades;?>"></td>
          </tr>
