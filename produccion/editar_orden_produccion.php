           ...
           <SCRIPT LANGUAGE="JavaScript">
			...
			function validar() {
				...
				if (document.registro.observaciones.value == "") {
					alert ('Debe escribir observaciones o desacripcion de la orden');
            ...
            <?php
              $db = new MySQL();
              $clave = $_GET['numero'];
			  $orden = $db->consulta("SELECT * FROM `orden_produccion` WHERE id_orden='".$clave."'");
              while ($row = $db->fetch_array($orden))
			  {
				  $fecha = $row['fecha'];
				  $fecha_entrega = $row['fecha_entrega'];
				  $observaciones = $row['observaciones'];
				  $id_sta = $row['id_estado'];
			...
			?>
                ...
                <form method="POST" action="registro_orden_produccion.php"  onSubmit="return validar()" name="registro" id="registro">
              ...
              <tr><td align="right">*Fecha entrega:</td>
                <td> <input class="texto" type="text" id="entrega" name="entrega" size="10" value="<?php echo $fecha_entrega;?>" /> <input type="checkbox" name="urge" id="urge" value="1" <?php if($urgente=="si"){echo "checked";}?>/>Urgente</td>
               </tr>
              ...
              <td align="right"><span>*</span>Producto: </td>
                  ...
                  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="nuevo_dis" id="nuevo_dis" value="1" <?php if($urgente=="si"){echo "checked";}?>/>Nuevo dise&ntilde;o
              </td>
              </tr>

               <tr><td align="right">Orden de compra Cliente:</td>
                <td> <input class="texto" type="text" id="orden" name="orden" size="30" value="<?php echo $orden_compra;?>" /> </td>
               </tr>
                
                <tr><td align="right" valign="top">Observaciones:</td>
                <td align="left"><textarea class="textarea" name="observaciones" id="observaciones" cols="120" rows="2"><?php echo $observaciones;?></textarea></td></tr>
                <tr>
                ...
                <td align="right"><span>*</span>Estado: </td>
                    ...
                    <select id="estado" name="estado">
                        <option value="-1">Selecciona Uno...</option>
                        <?php 
                            $lista_sta = $db->consulta("SELECT * FROM `status_orden_produccion`;");
