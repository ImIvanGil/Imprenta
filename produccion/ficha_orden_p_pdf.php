...
$queOrd = "SELECT * FROM `orden_produccion` WHERE id_orden='".$clave."'";
$orden = mysql_query($queOrd, $conexion) or die(mysql_error());
 ...
 while ($row = mysql_fetch_assoc($orden)) {
			...
			$id_cliente = $row['id_cliente'];
			  ...
			  // estado de la orden
			  $sql_status = "SELECT status FROM status_orden_produccion WHERE id_status='".$id_status."'";
...
function Header()
{
    ...
    $this->Cell(80);
    //TÃ­tulo
    $this->Cell(110,3,'ORDEN DE PRODUCCION',0,0,'R');
    //Salto de lÃ­nea
    $this->Ln(20);
}
...
$pdf->text($x+12,$y+47,'Orden de Compra: ');
$pdf->text($x+39,$y+47,$orden_compra);
...
$pdf->SetFont('Arial','B',9);
$dat5 = 'Fecha de Autorizacion: ';
$pdf->text($x+122,$y+29,$dat5);
$pdf->SetFont('Arial','',9);
$pdf->text($x+158,$y+29,$fecha_autoriza);
...
$pdf->SetFont('Arial','B',9);
$dat5 = utf8_decode('Tipo de diseño: ');
$pdf->text($x+122,$y+45,$dat5);
$pdf->SetFont('Arial','',9);
$pdf->text($x+146,$y+45,utf8_decode($diseno));
...
$pdf->text($x+173,$y+53,'CANTIDAD');
$pdf->SetFont('Arial','',9);

//recuadros para enmarcar el detalle de la orden
$pdf->SetLineWidth(0.09);
$pdf->Rect($x+10, $y+50, 21, 60,'D');
$pdf->Rect($x+31, $y+50, 135, 60,'D');
$pdf->Rect($x+166, $y+50, 29, 60,'D');
...
$sql_det = "SELECT * FROM detalle_orden_produccion WHERE id_orden='".$clave."'";
...
while ($row = mysql_fetch_assoc($consulta_det)){
	...
	$consulta_prod = mysql_query($sql_prod, $conexion) or die(mysql_error());
	 while ($row2 = mysql_fetch_assoc($consulta_prod)){
		...
		$query_papel = "SELECT *  FROM `insumo` WHERE `id_insumo` = '".$id_papel."';";
		$consulta_pap = mysql_query($query_papel);
		while ($row3 = mysql_fetch_assoc($consulta_pap)){$papel = $row3['nombre'];}
		...
		if($id_linea=="2"){
			...
			if($existe_copias<=0){
				...
				while ($row21 = mysql_fetch_assoc($copias))
				{
					...
					while ($row9 = mysql_fetch_assoc($lista_p)){$nombre_papel = $row9['nombre'];}
					$color = $row21['color'];
...
if($id_linea=="1"){
	...
	$pdf->text($x+35,$m+74,'Largo: ');
	$pdf->text($x+60,$m+74,'No. Dado: ');
	$pdf->text($x+90,$m+74,'Diametro Core: ');
	$pdf->text($x+125,$m+74,'Ancho Core: ');
	
	$pdf->text($x+11,$m+78,'Precorte: ');
	$pdf->text($x+35,$m+78,'Dientes del plate roll: ');
	$pdf->text($x+78,$m+78,'Repeticiones al eje: ');
	$pdf->text($x+78,$m+78,'Repeticiones al eje: ');
	$pdf->text($x+118,$m+78,'Repeticiones al desarrollo: ');

	$pdf->SetFont('Arial','',9);
	$pdf->text($x+25,$m+74,$ancho);
	$pdf->text($x+46,$m+74,$largo);
	$pdf->text($x+76,$m+74,$dado);
	$pdf->text($x+113,$m+74,$cored);
	$pdf->text($x+145,$m+74, $corea);
	$pdf->text($x+26,$m+78,$precorte);
	
	$pdf->text($x+68,$m+78,$dientes);
	$pdf->text($x+108,$m+78,$repeticionese);
	$pdf->text($x+159,$m+78,$repeticionesd);
	...
	$pdf->text($x+175,$m,number_format($cantidad,0));
...
if($existe_salidas>0){
	...
	$t =110;
	  while ($row = mysql_fetch_assoc($salidas))
	  {
		  ...
		  $cons_p = "SELECT * FROM `insumo` WHERE `id_insumo` =$id_papel;";
		  $lista_p = mysql_query($cons_p);
		  while ($row9 = mysql_fetch_assoc($lista_p)){$nom_salida = $row9['nombre'];}
